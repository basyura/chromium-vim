...
Visual.collapse = function () {
  ...
  if (
    (b === e && this.selection.extentOffset < this.selection.baseOffset) ||
    e < b
  ) {
...
Visual.scrollIntoView = function () {
  ...
  if (br.top < 0) {
    ...
    window.scrollBy(
      0,
      br.top + br.height - document.documentElement.clientHeight
    );
  }
};
...
Visual.enterLineMode = function () {
  ...
  if (this.selection.type === 'Caret') {
    ...
    if (
      bnode.parentNode.getBoundingClientRect().top >
      enode.parentNode.getBoundingClientRect().top
    ) {
...
Visual.action = function (key) {
  ...
  if (this.movements.hasOwnProperty(key)) {
    this.selection.modify.apply(
      this.selection,
      [movementType].concat(this.movements[key])
    );
    return;
  }
