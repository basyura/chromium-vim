...
Search.nextResult = function (reverse) {
  ...
  switch (Command.completionResults[this.index][0]) {
    ...
    case "topsites":
      Command.input.value =
        Command.input.value.match(/^\S+ /)[0] +
        Command.completionResults[this.index][2];
      break;
    case "tabhistory":
      Command.input.value =
        "tabhistory " + Command.completionResults[this.index][1];
      break;
    case "engines":
      Command.input.value =
        Command.input.value.match(/^\S+ /)[0] +
    ...
    case "search":
      ...
      if (Command.customCommands.hasOwnProperty(value[0])) {
        value = [value[0]];
        repl = Utils.split(Command.customCommands[value[0]], /\s+/)
          .slice(2)
          .join(" ");
      }
    ...
    case "windows":
      Command.input.value =
        Command.input.value.match(/^\S+/)[0] +
        " " +
        Command.completionResults[this.index][1].replace(/ .*/, "");
      break;
    case "chromesessions":
      Command.input.value =
        Command.input.value.match(/^\S+/)[0] +
        " " +
        Command.completionResults[this.index][3].replace(/ .*/, "");
      break;
    case "markOptions":
      Command.input.value = Command.input.value.replace(
        /-[a-zA-Z]*$/,
        Command.completionResults[this.index][1]
      );
      break;
    ...
    case "settings":
      ...
      Command.input.value =
        command[0] +
        " " +
        (/^no/.test(command[1]) ? "no" : "") +
        Command.completionResults[this.index][1];
      break;
    case "paths":
      if (Command.completionResults[this.index][2] !== "folder") {
        Command.input.value =
          "bookmarks " + Command.completionResults[this.index][2];
      } else {
        Command.input.value =
          "bookmarks " +
          Command.completionResults[this.index][3] +
          Command.completionResults[this.index][1];
      }
      break;
    case "buffers":
