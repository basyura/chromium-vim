...
Find.search = function (mode, repeats, ignoreFocus) {
  ...
  if (
    ...
    window.scrollBy(
      0,
      br.top * documentZoom + br.height * documentZoom - window.innerHeight
    );
...
Find.highlight = function (params) {
  ...
  nodes.forEach(
    (function () {
      if (useRegex) {
        return function (node) {
          ...
          matches.forEach(function (match) {
            ...
            if (node.data.length === 0) node.remove();
            if (end.data.length === 0) end.remove();
      ...
      return function (node) {
        var pos =
          containsCap || !settings.ignorecase
