...
var matchLocation = function (url, pattern) {
  // Uses @match syntax
...
Object.setPrototypeOf(TrieNode.prototype, {
  remove: function () {
    ...
    if (parent && Object.keys(this.children).length === 0) {
      ...
      if (parent.value === null) parent.remove();
    }
  },
  ...
  find: function (keys) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (!node.hasKey(keys[i])) return null;
...
var mapDOM = function (root, accept) {
  ...
  for (i = 1; i < nodes.length; i++) {
    ...
    if (value !== null) acceptedValues.push(value);
  }
...
var getLinkableElements = (function () {
  var visible = function (node) {
    ...
    return (
      cs.opacity !== "0" && cs.visibility === "visible" && cs.display !== "none"
    );
  };
  return function () {
