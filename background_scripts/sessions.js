...
Sessions.stepBack = function (sender) {
  if (
    Object.keys(tabHistory).length &&
    tabHistory[sender.tab.windowId] !== void 0 &&
    tabHistory[sender.tab.windowId].length > 0
  ) {
...
(function () {
  if (chrome.hasOwnProperty("sessions")) {
    ...
    if (chrome.sessions.hasOwnProperty("onchanged")) {
      // Chromium version 35 doesn't have this listener, but supports chrome.sessions
    ...
    chrome.tabs.onRemoved.addListener(function (id) {
      for (var key in Sessions.activeTabs) {
        if (Sessions.activeTabs[key].hasOwnProperty(id)) {
          ...
          tabHistory[Sessions.activeTabs[key][id].windowId].push(
            Sessions.activeTabs[key][id]
          );
